...
const { combine, colorize, json, errors, printf, timestamp, metadata, simple } = winston.format;

const PRODUCTION_ENV: boolean = process.env.NODE_ENV === "production";
const TESTING_ENV: boolean = process.env.NODE_ENV === "testing";

const devFormat = printf(({ timestamp, level, message, stack }: any) => `${timestamp} ${level}: ${stack || message}`);
...
const ConsoleTransport: transport = new winston.transports.Console({
  ...
  format:
    !PRODUCTION_ENV && !TESTING_ENV
      ...
      : combine(errors({ stack: "true" }), metadata({ fillExcept: ["message", "level", "timestamp"] }), prodFormat),
});
